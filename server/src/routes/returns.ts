...
export default async function returnRoutes(fastify: FastifyInstance) {
  fastify.get("/api/returns", async () => {
    return db("return_transactions")
      ...
      .orderBy("return_transactions.created_at", "desc");
  });

  fastify.get("/api/returns/:id", async (request) => {
  ...
  fastify.post("/api/returns", async (request) => {
  ...
  fastify.patch("/api/returns/:id/send-to-company", async (request) => {
    const { id } = request.params as { id: string };
    await db("return_transactions").where("id", id).update({ status: "sent_to_company", sent_to_company_date: db.fn.now() });
    return db("return_transactions").where("id", id).first();
  });

  fastify.patch("/api/returns/:id/receive-from-company", async (request) => {
    const { id } = request.params as { id: string };
  ...
  fastify.patch("/api/returns/:id/complete", async (request) => {
    const { id } = request.params as { id: string };
